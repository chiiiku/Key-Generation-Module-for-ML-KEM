
#ifndef __COMBINED__H
#define __COMBINED__H

#include "cynw_p2p.h"

#include "combined_input_type.h"
#include "combined_output_type.h"

SC_MODULE(combined)
{
    public:
    cynw_p2p < combined_INPUT_DT  >::in     inputs;
    cynw_p2p < combined_OUTPUT_DT >::out    outputs;
    
    // Declaration of clock and reset parameters
    sc_in_clk               clk;
    sc_in < bool >          rst;
    SC_CTOR(combined):inputs("inputs"), outputs("outputs"), clk("clk"), rst("rst")
    {

        SC_CTHREAD(thread1, clk.pos());
        set_stack_size(0x200000);
        reset_signal_is(rst,0);
        
        // Connect the clk and rst signals to the metaports
        inputs.clk_rst(clk, rst);
        outputs.clk_rst(clk, rst);
    }
    void thread1();
    
    combined_OUTPUT_DT my_function(combined_INPUT_DT);
};

#endif

